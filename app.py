...
async def get_file_details_api(request: Request, unique_id: str):
    ...
    if link_data.get('expire_at', 0) > 0 and int(time.time()) > link_data.get('expire_at'):
        await db.delete_link(unique_id) # Delete from database
        raise HTTPException(status_code=404, detail="Link expired.")
        
    message_id = link_data.get('message_id')
    client_ip = get_client_ip(request)
    
    # Generate Secure IP-based Token
    token_payload = {"ip": client_ip, "mid": message_id, "exp": int(time.time()) + (24 * 3600)} # Token valid for 24h
    secure_token = jwt.encode(token_payload, Config.SECRET_KEY, algorithm="HS256")
...
async def stream_media(r:Request, mid:int, fname:str, token:str=None):
    if not token: raise HTTPException(403, detail="Forbidden: Token missing. Do not hotlink.")
    try:
        decoded = jwt.decode(token, Config.SECRET_KEY, algorithms=["HS256"])
        if decoded.get("mid") != mid or decoded.get("ip") != get_client_ip(r):
            raise ValueError()
    except Exception:
        raise HTTPException(403, detail="Forbidden: Invalid or expired token. Generate a new link.")
