        ...
        if len(message.command) > 1 and message.command[1].startswith("verify_"):
            ...
            if Config.FORCE_SUB_CHANNEL:
                try:
                    ...
                    bot_username = client.me.username if client.me else Config.BOT_USERNAME
                    retry_button = InlineKeyboardButton("✅ Joined", url=f"https://t.me/{bot_username}?start={message.command[1]}")
                    ...
                    print(f"FSub Error: {e}") # Ignore if bot is not admin in Fsub
                    pass
            ...
            reply_text = f"👋 **Hello, {user_name}!**\n\n__Welcome To Sharing Box Bot. Just Send Or Forward Any File To Me!__"
            await message.reply_text(reply_text)
            
    except Exception as e:
        print(f"!!! START CMD ERROR: {traceback.format_exc()}")
...
async def handle_file_upload(message: Message):
    try:
        ...
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("10 Mins", callback_data=f"exp_{sent_msg.id}_10"),
             InlineKeyboardButton("1 Hour", callback_data=f"exp_{sent_msg.id}_60")],
            [InlineKeyboardButton("24 Hours", callback_data=f"exp_{sent_msg.id}_1440"),
             InlineKeyboardButton("Permanent", callback_data=f"exp_{sent_msg.id}_0")]
        ])
        await message.reply_text("**⏳ Select Link Expiry Time:**\n\n__Choose how long this link will be valid.__", reply_markup=kb, quote=True)
    except Exception as e:
...
@bot.on_callback_query(filters.regex(r"^exp_"))
async def set_expiry_callback(client: Client, query):
    _, msg_id, mins = query.data.split("_")
    unique_id = secrets.token_urlsafe(8)
    
    expire_at = 0
    if int(mins) > 0:
        expire_at = int(time.time()) + (int(mins) * 60)
        
    await db.save_link(unique_id, int(msg_id), expire_at)
    bot_username = client.me.username if client.me else Config.BOT_USERNAME
    verify_link = f"https://t.me/{bot_username}?start=verify_{unique_id}"
    
    btn = InlineKeyboardMarkup([[InlineKeyboardButton("🔗 Get Link Now", url=verify_link)]])
    exp_text = "Permanent" if int(mins) == 0 else f"{mins} Minutes"
    await query.message.edit_text(f"__✅ File Uploaded!__\n⏱ **Expiry:** `{exp_text}`", reply_markup=btn)

@bot.on_message(filters.command("stats") & filters.user(Config.OWNER_ID))
async def bot_stats(_, message: Message):
    users, links = await db.get_stats()
    await message.reply_text(f"📊 **Bot Statistics:**\n\n👥 Total Users: `{users}`\n🔗 Total Links Generated: `{links}`")

@bot.on_message(filters.command("broadcast") & filters.user(Config.OWNER_ID))
async def bot_broadcast(_, message: Message):
...
@app.get("/show/{unique_id}", response_class=HTMLResponse)
async def show_page_safe(request: Request, unique_id: str):
    try:
        return templates.TemplateResponse("show.html", {"request": request})
    except Exception as e:
        return HTMLResponse(content=f"<h3>UI Loading Error: {e}</h3>", status_code=500)

@app.get("/api/file/{unique_id}", response_class=JSONResponse)
async def get_file_details_api(request: Request, unique_id: str):
    try:
        link_data = await db.get_link(unique_id)
        if not link_data:
            raise HTTPException(status_code=404, detail="Link invalid.")
            
        # If fallback needed
        if isinstance(link_data, int):
            link_data = {'message_id': link_data, 'expire_at': 0}
            
        # Auto Delete / Expire Check
        if link_data.get('expire_at', 0) > 0 and int(time.time()) > link_data.get('expire_at'):
        ...
        secret = getattr(Config, "SECRET_KEY", "super_secret_key_123")
        token_payload = {"ip": client_ip, "mid": message_id, "exp": int(time.time()) + (24 * 3600)}
        secure_token = jwt.encode(token_payload, secret, algorithm="HS256")
    ...
    dl_base = f"{Config.BASE_URL}/dl/{message_id}/{safe_file_name}?token={secure_token}"
    response_data = {
        "file_name": mask_filename(file_name),
        "file_size": get_readable_file_size(media.file_size),
        "is_media": mime_type.startswith(("video", "audio")),
        "direct_dl_link": dl_base,
        "mx_player_link": f"intent:{dl_base}#Intent;action=android.intent.action.VIEW;type={mime_type};end",
        "vlc_player_link": f"intent:{dl_base}#Intent;action=android.intent.action.VIEW;type={mime_type};package=org.videolan.vlc;end"
    }
...
async def stream_media(r:Request, mid:int, fname:str, token:str=None):
    if not token: raise HTTPException(403, detail="Forbidden: Token missing.")
    try:
        secret = getattr(Config, "SECRET_KEY", "super_secret_key_123")
        decoded = jwt.decode(token, secret, algorithms=["HS256"])
        if decoded.get("mid") != mid or decoded.get("ip") != get_client_ip(r):
            raise ValueError("IP or ID mismatch")
    except Exception as e:
        print(f"TOKEN ERROR: {e}")
        raise HTTPException(403, detail="Forbidden: Invalid or expired token.")
